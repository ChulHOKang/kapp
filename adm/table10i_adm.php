<?php
	...
	if( $H_LEV < 8) {
		m_("admin page");		//echo("<meta http-equiv='refresh' content='0; URL=./'>");
		exit;
	}
	...
	/*  2021-04-08
		table10i_adm.php : table-{$tkher['table10_table']}

			- g5/pg/url_list_all.php : /g5/pg/ 에서 여기 /cratree/ 로 이동.

	*/
	$day = date("Y-m-d H:i:s");
	$pg_		= 'table10i_adm.php';
?>
<html>
<head>
...
<?php
   ...
   if( $H_ID && $mode == 'Delete_mode' ) {
		...
		if( !$tab_enm ) {
			...
			$result = sql_query( "SELECT * from {$tkher['table10_table']} where tab_enm='$tab_enm' and fld_enm='seqno' " );//tab_enm asc,
		...
		if( strlen($mid) > 0 ) {
			...
			if( strlen($pj_code) > 0 ) $ls = $ls . " and group_code= '".$pj_code."'";
		} else if( strlen($pj_code) > 0 ) $ls = $ls . " and group_code= '".$pj_code."'";
...
<!--
	...
	function group_name_add_func(){
		...
		if ( window.confirm( msg ) )
		{
			...
			document.table_list.action = "table10i_adm.php";
	...
	function tkher_source_create(hnm, enm, $coin){
		if( $coin < 1000 ) {
			alert('Point is low. You must do activities to accumulate points. point:'+ $coin);//Point가 부족합니다. point를 축적해야합니다.
			return false;
		} else {
	...
	function kmember(mid) {
		...
		document.table_list.mid_nm.value= arr[num].text;
		document.table_list.pj_code.value='';
		document.table_list.action="table10i_adm.php";
		document.table_list.submit();
	}
...
<?php
			if( strlen($mid) > 0 ) echo "<option value='".$mid."' selected >".$mid_nm."</option>";
...
<?php
	if( strlen($pj_code) > 0 ) echo "<option value='".$pj_code."' selected >".$pj_name."</option>";
	if( strlen($mid)>0 ) $sql ="SELECT * from {$tkher['table10_group_table']} where userid ='".$mid."'";
...
?>
			...
			<TD <?php echo "title='Prints a list of columns.' "; ?> ><a href="javascript:table_sel_func('<?=$rs['tab_enm']?>', '<?=$rs['tab_hnm']?>', '<?=$data?>', '<?=$Table_page?>' );"><?=$rs['tab_hnm']?><img src="<?=KAPP_URL_T_?>/icon/default.gif"></a></TD>
			<TD <?php echo "title='Prints a list of columns.' "; ?> ><a href="javascript:table_sel_func('<?=$rs['tab_enm']?>', '<?=$rs['tab_hnm']?>', '<?=$data?>', '<?=$Table_page?>' );"><?=$rs['tab_enm']?></a></TD>
			<TD><?=$rs['upday']?></TD>
<?php
		}else if( $mode =='Search'){
?>
...
?>
		<table width="100%">
		  <th>SQL</th>
		<th><textarea id='pg_relation' name='item_array' style="border-style:;background-color:black;color:yellow;height:90px;width:100%;" readonly><?=$sql?></textarea></th>
		</table>
<?php
	}
?>
...
<?php
	if( $mode =='Search' ) {
		...
		$first_page = intval(($Table_page-1)/$page_num+1)*$page_num-($page_num-1);

<?php
	...
	/*
	 * table_design_update_adm.php : call - kapp_table_list_adm.php
		: table_design_update.php copy
...
<script language=javascript>
	function table_name_change(){ // add 2023-09-08 kan
		...
		if( tab_hnmS == "Select table" || !tabhnm ){
			...
			if (ins)
			{
				...
				document.insert.action="table_design_update_adm.php";
	...
	function resetgo(){
		window.open( 'table_design_update_adm.php' , '_self', '');
		return;
	}

	function table_list(){
		window.open( 'kapp_table_list_adm.php' , '_self', '');
		return;
	}
	...
	function Save_Update_Insert(line){ // line number reset
		if( !document.insert.userid.value) {
			alert(' Please login! ');
			return false;
		}
...
//		$userid	= $H_ID;
	...
	if( $mode == 'Search' ){
			...
			//$tab_enm = $aa[0];
			//$tab_hnm = $aa[1];
			$result = sql_query( "SELECT * from {$tkher['table10_table']} where userid='$mid' and tab_enm='$tab_enm' order by disno" );
...
?>
	<Form METHOD='POST' name='insert' enctype="multipart/form-data">
		...
		<input type="hidden" name="mid" value='<?=$mid?>'> <!--  H_ID -->
		...
		<h2><font fce="Arial">Table Design Update <?php if( $mode=='Search' ) echo "( Change Design )"; ?></font></h2>
...
<?php
			$SQLG = "SELECT * from {$tkher['table10_group_table']} where userid='".$mid."' order by group_name ";
...
<?php
		...
		$result = sql_query( "SELECT * from {$tkher['table10_table']} where userid='".$mid."' and fld_enm='seqno'  order by upday desc");	//group by tab_enm " );
		while( $rs = sql_fetch_array($result)) {
?>
...
<?php
		if( $mode=="Search") {
?>
			<input <?php echo "title='table list screen.' "; ?> type='button' name='Back' onclick="javascript:table_list();"
			value="Back table list" style='height:25px;background-color:black;color:white;border-radius:20px;border:1 solid white'>&nbsp;&nbsp;&nbsp;
...
<?php
	...
	function TAB_curl_move( $tab_enm, $tab_hnm, $fld_enm, $fld_hnm, $fld_type, $fld_len, $cnt, $memo, $Asqltable, $Aif_line, $Aif_type, $Aif_data, $Arelation_data ){
		global $tabData, $mid, $H_EMAIL, $group_code, $group_name;
	...
	if( $del_mode == 'column_modify_mode' ){ //
		...
		if( $table_yn =='y' ) {
			...
			$sql = "INSERT INTO {$tkher['table10_table']} set  tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='$fld_enm', fld_hnm='$fld_hnm', fld_type='$fld_type', fld_len=$fld_len, userid='$mid', group_code='$group_code', group_name='$group_name', memo='$fld_memo', disno=$dis ";
			$ret = sql_query( $sql );
		...
		if( $table_yn =='y' ) {
			...
			$mq1 =sql_query($query);
			if( $mq1 ) {
				sql_query( "INSERT INTO {$tkher['table10_table']} set  tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='$fld_enm', fld_hnm='$fld_hnm', fld_type='$fld_type', fld_len=$fld_len, userid='$mid', group_code='$group_code', group_name='$group_name', memo='$fld_memo', disno=$dis " );
				m_(" column add OK!! ");
			}
			else {
		...
		$mq1	=sql_query($query);
		$query = "DELETE from {$tkher['table10_table']} where seqno = ". $_POST["del_seqno"];
		$mq2	=sql_query($query);
		echo "<script>create_after_run( '$tab_enm' , '$tab_hnm' , '$del_mode' );</script>";
	}
	...
	function create_reaction_func(){
		global $mid, $tab_enm, $mode, $tab_hnm, $group_code, $group_name;
		global $config;
		global $tkher;
		$query	= " DELETE from {$tkher['table10_table']} where tab_enm='".$tab_enm."' and userid='".$mid."' ";
		$mq1	= sql_query($query);
		...
		For( $ARR=1; $_POST["fld_hnm"][$ARR] ; $ARR++ ) {
			...
			if( $fld_hnm ) {
				...
				sql_query( "INSERT INTO {$tkher['table10_table']} set  group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='$fld_enm', fld_hnm='$fld_hnm', fld_type='$fld_type', fld_len='$fld_len', disno=$ARR, userid='$mid', table_yn='y', memo='$memo' " );
				$cnt++;
			}
		}
		$item_list = $item_list . " primary key(seqno) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		sql_query( "INSERT INTO {$tkher['table10_table']} set  group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='seqno', fld_hnm='seqno', fld_type='INT', fld_len='10', disno=$cnt, userid='$mid', table_yn='y', memo='key column', sqltable='$item_list' " );
		$line_set = $cnt;
		...
		if( $table10_pg ) {
			...
			$query="INSERT INTO {$tkher['table10_pg_table']} SET group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm',tab_hnm='$tab_hnm', pg_code='$tab_enm', pg_name='$tab_hnm', item_array='$item_array', if_type='$if_type', if_data='$if_data', item_cnt=$cnt,  userid='$mid' ";
			sql_query($query);
			$link_ = KAPP_URL_T_ . "/table_design_update_adm.php";
	...
	function create_func(){
		global $mid, $tab_enm, $table_yn, $mode, $line_set, $ip, $group_code, $group_name;
		global $config;
		global $tkher;
		$item_list = " create table ". $tab_enm . " ( ";
		$item_list = $item_list . " seqno int auto_increment not null, ";
		...
		For( $ARR=1; $ARR < $line_set ; $ARR++ ) {
			...
			if( $fld_hnm ) {
				...
				$sql = "INSERT INTO {$tkher['table10_table']} set  tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='$fld_enm', fld_hnm='$fld_hnm', fld_type='$fld_type', fld_len='$fld_len', disno=$ARR, userid='$mid', table_yn='y', group_code='$group_code', group_name='$group_name', memo='$memo' ";
				$ret = sql_query( $sql );
				if( !$ret ) {
					echo "sql: " . $sql; exit;
				}
		...
		$item_list = $item_list . " primary key(seqno) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		$line_set  = $cnt - 1;
		sql_query( "INSERT INTO {$tkher['table10_table']} set  tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='seqno', fld_hnm='seqno', fld_type='INT', fld_len='10', disno=0, userid='$mid', table_yn='y', group_code='$group_code', group_name='$group_name', memo='$item_array', sqltable='$item_list' " );
		...
		$sqlPG		= "SELECT * from {$tkher['table10_pg_table']} where userid='$mid' and pg_code='$tab_enm' ";
		$resultPG	= sql_query($sqlPG);
		$table10_pg = sql_num_rows($resultPG);
		if( $table10_pg ) {
			$query="UPDATE {$tkher['table10_pg_table']} SET group_code='$group_code', group_name='$group_name',  item_cnt=$line_set, item_array='$item_array' WHERE userid='$mid' and pg_code='$tab_enm' ";
			sql_query($query);
		} else {
			$query="INSERT INTO {$tkher['table10_pg_table']} SET group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm',tab_hnm='$tab_hnm', pg_code='$tab_enm', pg_name='$tab_hnm', item_array='$item_array', if_type='$if_type', if_data='$if_data', item_cnt=$line_set,  userid='$mid' ";
			$ret = sql_query($query);
			if( $ret ){
	...
	function update_remake_func(){
		global $mid, $tab_enm, $mode,$group_code,$group_name;
		global $config;
		global $tkher;
		$query	="delete from {$tkher['table10_table']} where tab_enm='$tab_enm' and userid='$mid' ";
		$mq1	=sql_query($query);
		...
		$item_list = $item_list . " primary key(seqno) ) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		sql_query( "INSERT INTO {$tkher['table10_table']} set  group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm', tab_hnm='$tab_hnm', fld_enm='seqno', fld_hnm='seqno', fld_type='INT', fld_len='10', disno=0, userid='$mid', table_yn='y', memo='$item_array', sqltable='$item_list' " );
		...
		if( $table10_pg ) {
			m_(" ok pg - update 1");
			$query="UPDATE {$tkher['table10_pg_table']} SET item_cnt=$line_set, item_array='$item_array' WHERE pg_code='$tab_enm' ";
			sql_query($query);
		} else {
			$query="INSERT INTO {$tkher['table10_pg_table']} SET group_code='$group_code', group_name='$group_name', tab_enm='$tab_enm',tab_hnm='$tab_hnm', pg_code='$tab_enm', pg_name='$tab_hnm', item_array='$item_array', if_type='$if_type', if_data='$if_data', item_cnt=$line_set,  userid='$mid' ";
			sql_query($query);

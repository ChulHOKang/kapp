<?php
	...
	/*
		/t/adm/ktree_list_adm.php - /t/tree_menu_list_guest.php copy
		/t/tree_menu_list_guest.php : /t/menu/tree_menu_list.php copy Guest용 tree list
		/t/menu/tree_menu_list.php : /t/menu/ktree_list_adm.php copy 모바일용 통합 tree list
		2021-05-30 : 모바일용 생성
	*/
...
<?php
	...
	if( $mode=='MySearch' ){
		if($sel=='like') $data = '%' . $data . '%';
		$query = "SELECT * from {$tkher['sys_menu_bom_table']} where $param $sel '$data' and sys_level='mroot' and sys_subtit != 'main' order by tit_gubun desc, up_day desc, sys_subtit $limit";
	} else {
...
while ($line = sql_fetch_array( $result )) {
	...
	if( $line['tit_gubun']=='G') { // board
		...
		$bb='green';
		$iconX="<img src='".KAPP_URL_T_."/logo/ship.png' width='20' height='15' title='Tkher-Tree Board:'>";
		$typeT = 'board';
		...
		$bb='yellow';
		$run_mode = 'cratree_book_remake';
		$iconX="<img src='".KAPP_URL_T_."/logo/pizza.png' width='20' height='15' title='Tkher-Tree Note:'>";
		$typeT = 'note';
	} else if( $line['tit_gubun']=='T'  ) { // tree
		$bb='#99CCFF';
		$run_mode = 'cratree_remake';
		$iconX="<img src='".KAPP_URL_T_."/logo/berry.png' width='20' height='15' title='Tkher-Tree Url:'>";
		$typeT = 'link';
	} else {
		$run_mode = 'cratree_remake';
		$iconX='';
	...
	if ( $H_ID and $mid == $from_session_id or $h_lev > 7 ) {
		...
		<tr>
			...
			<td><a href='$run' target='_blank' style='color:$bb' title='maker:".$mid.", view:".$line['view_cnt']."' style='color:white;'>".$line['sys_subtit']."</a></td>
			<td align='center'><a href='$runM' target='_blank' style='color:blue' title='Launch the popup menu:$runM'>Popup</a>	</td>
			<td><input type='button' value='Tree DN' onclick=\"treeDN_func('$mid', '$sys_pg', '$run_mode', '$H_POINT');\" style='background-color:black;color:white;' title='Download source code of $subtit'>
			</td>
			<td>

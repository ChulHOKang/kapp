<?php
	...
	/*
		kapp_permission_adm.php : app grant level setting.
...
$(function () {
	...
	document.getElementById('tit_et').addEventListener('click', function(e) {
		...
		timer = setTimeout(() => {
			switch(e.target.innerText){
				case 'Project' : title_func('group_name', 'kapp_permission_adm.php'); break;
				case 'User'    : title_func('userid', 'kapp_permission_adm.php'); break;
				case 'Program' : title_func('pg_name', 'kapp_permission_adm.php'); break;
				case 'Table'   : title_func('tab_hnm', 'kapp_permission_adm.php'); break;
				case 'Date'    : title_func('upday', 'kapp_permission_adm.php'); break;
				default        : title_func('', 'kapp_permission_adm.php'); break;
			}
		}, 250);
	});
	document.getElementById('tit_et').addEventListener('dblclick', function(e) {
		clearTimeout(timer);
		switch(e.target.innerText){
				case 'Project' : title_wfunc('group_name', 'kapp_permission_adm.php'); break;
				case 'User'    : title_wfunc('userid', 'kapp_permission_adm.php'); break;
				case 'Program' : title_wfunc('pg_name', 'kapp_permission_adm.php'); break;
				case 'Table'   : title_wfunc('tab_hnm', 'kapp_permission_adm.php'); break;
				case 'Date'    : title_wfunc('upday', 'kapp_permission_adm.php'); break;
				default        : title_wfunc('', 'kapp_permission_adm.php'); break;
		}
	});
...
<!--
	/*function title_wfunc( thisform, fld_code_pg){       
		...
		thisform.action='kapp_permission_adm.php';
		thisform.submit();                         
	} 
	function title_func( thisform, fld_code){       
		...
		document.kapp_program_list_Form.action='kapp_permission_adm.php';
	...
	function run_func( pg ){
		document.kapp_program_list_Form.action='<?=KAPP_URL_T_?>/kapp_program_data_list.php';
		...
		if( resp === true ) {
			jQuery(document).ready(function ($) {
				$.ajax({
					...
					method: "post",
						url: '<?=KAPP_URL_T_?>/kapp_column_change_ajax.php',
						data: {
...
<?php
	...
	if( $mode == "update_level") {
		...
		$sel_r = $_POST['read_'];
		$sel_w = $_POST['write_'];
		$query = "update {$tkher['table10_pg_table']} set grant_view='$sel_r', grant_write='$sel_w' where seqno=$seqno  ";
		$mq = sql_query($query);
		...
		$ls = " SELECT * from {$tkher['table10_pg_table']} ";
		if( $data !=''){
			if( $sel == 'like' ) $wsel = " where $param $sel '%$data%' ";
			else $wsel = " where $param $sel '$data' ";
		} else if( $group_code!='') $wsel = " where group_code = '$group_code'";
		else $wsel = '';
		if( $wsel!='' ) $ls = $ls . $wsel;
...
, View Line: 
<SELECT id='line_cnt' name='line_cnt' onChange="kapp_line_cnt_submit('kapp_permission_adm.php')" style='height:20;'>
...
<?php
	paging("kapp_permission_adm.php",$total_count,$page,$line_cnt, "document.kapp_program_list_Form"); 
?> 

<?php
	...
	/*
		kapp_permission_adm.php : kapp_permission_adm.php - app grant level setting.
	*/
...
<!--
	function title_func(fld_code){       
		...
		document.insert_form.action='kapp_permission_adm.php';
...
<?php
	...
	if( $mode == "update_level") {
		...
		if( $data !='' ){
			if( $sel == 'like' ) $ls = $ls . " where $param $sel '%$data%' ";
			else $ls = $ls . " where $param $sel '$data' ";
			if( $wsel!='' ) $ls = $ls . $wsel;
		} else if( $wsel!='' ) $ls = $ls . " where group_code='$group_code' ";
...
<center>
<?php
		//if( $mode=="Search" ) $T_msg = "[ KAPP permissions : ". $pg_name . " ]";
		//else $T_msg = "[ Set App permissions ]";
		$T_msg = "[ Admin KAPP permissions ]";
...
<!-- <span title='my data print - kapp_permission_adm.php'><strong><a onclick="javascript:my_data();" style="border-style:;background-color:black;color:yellow;height:28;border-radius:20px;"><?=$T_msg?></a></strong></span> -->

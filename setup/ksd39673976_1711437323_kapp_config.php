<?php
	...
	if( $mode == 'record_update' ) {
 			$query			= " UPDATE ksd39673976_1711436495 SET  ";
			$ff_nm = time() . '_';
			$f_path = './' . $ff_nm;   // $f_path='./file/';  // dir add
 			$query = $query . "kapp_title = '" . $_POST['kapp_title '] . "' ";
 			$query = $query . ",kapp_theme= '" . $_POST['kapp_theme'] . "' ";
 			$query = $query . ",kapp_admin= '" . $_POST['kapp_admin'] . "' ";
 			$query = $query . ",kapp_admin_email= '" . $_POST['kapp_admin_email'] . "' ";
 			$query = $query . ",kapp_admin_email_nam= '" . $_POST['kapp_admin_email_nam'] . "' ";
 			$query = $query . ",kapp_use_point= '" . $_POST['kapp_use_point'] . "' ";
 			$query = $query . ",kapp_point_term= '" . $_POST['kapp_point_term'] . "' ";
 			$query = $query . ",kapp_login_point= '" . $_POST['kapp_login_point'] . "' ";
 			$query = $query . ",kapp_read_point= '" . $_POST['kapp_read_point'] . "' ";
 			$query = $query . ",kapp_write_point= '" . $_POST['kapp_write_point'] . "' ";
 			$query = $query . ",kapp_comment_point= '" . $_POST['kapp_comment_point'] . "' ";
 			$query = $query . ",kapp_download_point= '" . $_POST['kapp_download_point'] . "' ";
 			$query = $query . ",kapp_register_point= '" . $_POST['kapp_register_point'] . "' ";
 			$query = $query . ",kapp_recommend_point= '" . $_POST['kapp_recommend_point'] . "' ";
 			$query = $query . ",kapp_memo_send_point= '" . $_POST['kapp_memo_send_point'] . "' ";
			...
			echo "<script>window.open( './ksd39673976_1711437323_run.php' , '_self', ''); </script>";
	}        */
	$menu1TWPer=15;
...
function table_data_list($pg_code) {
    document.tkher_form.action = "ksd39673976_1711437323_run.php?pg_code=" + $pg_code;
...
function record_modify($pg_code, seqno) {
    if (confirm("Do you want to change it?")) {
        ...
        document.makeform.action = 'ksd39673976_1711437323_update.php?pg_code=' + $pg_code;
...
<body width=100%>
    <center>
        ...
        <?php
	//$SQLX = " SELECT * from {$tkher['config_table']} where kapp_title = 'K-App' ";   // kapp_title 키로 사용
    $SQLX = " SELECT * from {$tkher['config_table']} ";  // kapp_title 사용안함.
...
?>
        <div class="HeadTitle01AX">
            <P href='#' class='on' title='table code:kapp_config , program name:kapp_config_2'>kapp_config
            </P>
        </div>
        <form name='tkher_form' method='post' enctype='multipart/form-data'>
            <input type=hidden name='mode' value='' />
            <input type=hidden name='seqno' value='' />
            <input type=hidden name='pg_name' value='kapp_config_2' />
            <input type=hidden name='pg_code' value='ksd39673976_1711437323' />
        ...
        <div class='boardViewX'>
            ...
            <div class='viewSubjX'><span>kapp_config_2</span> </div>
            ...
            <form name='makeform' action='' method='post' enctype='multipart/form-data' onsubmit='return check(this)'>
                ...
                <div class='menu1T' align=center><span
                        style='width:<?=$Xwidth?>;height:<?=$Xheight?>;'>kapp_title</span></div>
                ...
                <div class='blankA'> </div>
                <div class='menu1T' align=center><span
                        style='width:<?=$Xwidth?>;height:<?=$Xheight?>;'>kapp_theme</span></div>
                ...
                <div class='viewHeader'>
                    <!-- <input type=button value='Save'
                        onclick="javascript:record_modify('ksd39673976_1711437323','<?=$seqno?>');" class='Btn_List02A'>
                    <input type=button value='List' onclick="javascript:tab_pg_list('ksd39673976_1711437323');"
                        class='Btn_List02A'> -->
                </div>
            </form>
        </div>

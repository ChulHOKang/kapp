<?php
	...
	/*
        ...
        first setup -> kapp_dbcon_create.php 에서 테이블을 생성한다.
	...
	*/
	...
	if( isset($_SESSION['mb_level']) ) $H_LEV = $_SESSION['mb_level'];

	$url = 'DB_Table_CreateA.php?admin=modumoa';
    ...
    if( isset($_POST['prefix']) ) {
		...
		$prefix = KAPP_TABLE_PREFIX;
		$_SESSION['table_prefix'] = KAPP_TABLE_PREFIX;
	}

	DB_Connect();
    if( $_SESSION['mb_level'] > 7 && $mode == 'create_all') {
        $uncreate_table_list = json_decode( getJsonText( $_POST['uncreate_table_list']), true);
        Table_Create_All($prefix, $uncreate_table_list);
        Replace();
        exit;
    } else if( $_SESSION['mb_level'] > 7 && $mode == 're_create_all') {
        $create_table_list = json_decode(getJsonText($_POST['create_table_list']), true);
        Table_Re_Create_All($prefix, $create_table_list);
        Replace();
        exit;
        ...
        foreach( $create_table_list as $table){   
			...
			Delete_table($prefix, $table);
        }
        Replace();
        exit;
    } else if( $_SESSION['mb_level'] > 7 ) {
        if( $_SESSION['mb_level'] < 7 && !isset($_POST['admin'])) { 
	...
	function DB_Connect(){
		...
		if( file_exists( $kapp_dbcon_connect ) ) {  
			...
			m_("KAPP Setup please! No found file: " . $kapp_dbcon_connect);
...
<?php if( $H_LEV > 7 ) { ?>
				<div class="div_tab" style="text-align:center;"><input type="button" style='background-color:red;color:yellow;' onclick="Delete_Table_All()" value="Delete All Table" title='Data and Table Delete All - Requires a fresh install '></div>
<?php } ?>
...
<?php
	...
	foreach( $table_list as $table ) { 
        ...
        if( $table['table_link'] != '') {
			$href = 'a href = ./'. $table['table_link'];
			$back_ground_colorL = 'style="background-color:yellow;"';
		}
        $is_table = Table_Seatch( $prefix, $table['table_name']);
        if( $is_table) {
            $state = 'Created - ' . $cnt++;
            $t_check = 'checked';
...
function Create_Table_RE(_table_name) { // 기존 데이터가 전부 삭제됩니다. 테이블을 재생성하시겠습니까?
    if( confirm("All existing data will be deleted. Would you like to recreate the table?")) {
        $.ajax({
            type: "post",
            dataType: "json",
            data: {
...
function Delete_Table(_table_name) {
	alert("Del _table_name: " + _table_name + ", pref: " +JSON.stringify($("#prefix").val()));
...
function Create_Table_All() {
    let uncreate_table_list_array = '<?=$uncreate_table_list_array?>';
...
function Re_Create_Table_All() {
    let create_table_list_array = '<?=$create_table_list_array?>';

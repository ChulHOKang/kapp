...
session_start();
/*
    kapp_start_necessary.php : call : indexTT.php
	/t/index.php - include : kapp_start_necessary_TT.php - old: tkher_start_necessary.php
	 ...
	 모든 Program    - include : kapp_start_necessary.php
*/
...
$member		= array();
	...
	$dbconfig_file = KAPP_PATH_T_ . "/data/kapp_dbcon.php"; // "/data/tkher_dbconfig.php";
...
//https://ailinkapp.com/t/t/adm/index.php, start REF: , SC: /t/adm/index.php, SCF: /var/www/html/t/adm/index.php
//https://ailinkapp.com/t/t/indexTT.php, start REF: https://ailinkapp.com/t/adm/, SC: /t/indexTT.php, SCF: /var/www/html/t/indexTT.php
	...
	//m_("SESSION : ".$_SESSION['ss_mb_id']);
	if( isset($_SESSION['ss_mb_id']) ) { // 로그인중이라면
		$LOGIN_TYPE = get_session('urllink_login_type');
		if( $LOGIN_TYPE == "appgeneratorsystem"){
			$member = get_urllink_memberA( $_SESSION['ss_mb_id']);           // id- my_func , get_member(id) - tkher_common.lib.php
		...
		if( $member['mb_intercept_date'] && $member['mb_intercept_date'] <= date("Ymd", KAPP_SERVER_TIME)) {
			...
			if( substr( $member['mb_today_login'], 0, 10) != KAPP_TIME_YMD) { // 첫 로그인 포인트 지급
...
//=========================================================================================
// 회원, 비회원 구분
if( $member['mb_level'] >= $config['admin_level']) $is_admin = 'super'; // 7 -> $config['admin_level'] - 2024-02-19
else {
	$is_admin = '';
}
...
//if( get_cookie('kapp_ck_visit_ip') != $H_ID_IPADDR ) {
	include_once('./kapp_visit_insert.php'); // include_once('./visit/visit_insert.inc.php'); // 로그인 log 사용
//}

//m_("get_cookie - kapp_ck_visit_ip: " . get_cookie('kapp_ck_visit_ip') . ", _SERVER-REMOTE_ADDR: " . $_SERVER['REMOTE_ADDR'] . ", H_ID_IPADDR: " . $H_ID_IPADDR . ", HTTP_REFERER: " . $_SERVER['HTTP_REFERER']); //--- SCRIPT_NAME: /t/indexTT.php
//m_("his:".KAPP_TIME_HIS . ", get_cookie - kapp_ck_visit_ip: " . get_cookie('kapp_ck_visit_ip') . ", H_ID_IPADDR: " . $H_ID_IPADDR); //--- SCRIPT_NAME: /t/indexTT.php
//his:17:36:48, get_cookie - kapp_ck_visit_ip: ailinkapp.com_dao_172.31.34.132, H_ID_IPADDR: ailinkapp.com_dao_172.31.34.132
//his:17:12:18, get_cookie - kapp_ck_visit_ip: , H_ID_IPADDR: ailinkapp.com_dao_172.31.5.156
//get_cookie - kapp_ck_visit_ip: , _SERVER-REMOTE_ADDR: 172.31.5.156, H_ID_IPADDR: ailinkapp.com_dao_172.31.5.156, HTTP_REFERER: https://ailinkapp.com/t/index.php
//get_cookie - kapp_ck_visit_ip: , _SERVER-REMOTE_ADDR: 172.31.34.132, H_ID_IPADDR: ailinkapp.com_dao_172.31.34.132, HTTP_REFERER: https://ailinkapp.com/t/index.php
//get_cookie - kapp_ck_visit_ip: , _SERVER-REMOTE_ADDR: 172.31.5.156, H_ID_IPADDR: ailinkapp.com_dao_172.31.5.156, HTTP_REFERER: https://ailinkapp.com/t/index.php
...
$KAPP_URL=KAPP_URL;
$KAPP_PATH=KAPP_PATH;
$KAPP_CAPTCHA_URL=KAPP_CAPTCHA_URL;
$KAPP_CAPTCHA_PATH=KAPP_CAPTCHA_PATH;

...
<?php
	...
	*/
	...
	if( isset($data_R[$relation_num]) && $data_R[$relation_num]!='') $rel_ = explode("$", $data_R[$relation_num] );
	if( $relation_type_key !=''){
		...
		if( isset($typeR[1]) && $typeR[1] !=''  ){
			$type_R  = explode("@", $typeR[1]);
...
<?php 
		...
		if( $mode=='pg_codeS_Search' || $mode=='Relation_SearchTAB' || isset($_SESSION['Rtab_hnmS']) || isset($_SESSION['pg_codeS']) )
...
?>
 ...
 <input type='radio' onclick='radio_box_func(<?=$relation_num?>,this.value)' id='relation_type_SQL' name='relation_type_SQL' value='Insert' <?php if( isset($key_[0]) && $key_[0] =='Insert') echo 'checked'; ?> >Insert</label>
 <?php echo "<label style='background-color:$rel_cB;' title='Update must set the key column.' >"; ?>
 <input type='radio' onclick='radio_box_func(<?=$relation_num?>,this.value)' id='relation_type_SQL' name='relation_type_SQL' value='Update' <?php if( isset($key_[0]) && $key_[0] =='Update') echo 'checked'; ?> >Update</label>

...
<script language="javascript">
	...
	function new_create(Anum) {
		if( confirm('Are you sure you want to create? ') ) {
			location.href='../menu/index_create.php';
		}
	}
	function treeDN_func( mid, sys_pg, run_mode, point ){
		...
		document.sys_form.action='../menu/treebom_tree_menu_createDN.php';
	...
	function popupDN_func(mid, sys_pg, run_mode, point ){
		...
		document.sys_form.action='../menu/treebom_dropdown_menu_createDN.php';
		document.sys_form.target  = "_blank";
		document.sys_form.submit();
	}
...
if( $total > 0 ) {
	...
	if( $fld_code!='' ) $OrderBy = " order by tit_gubun desc, $fld_code ";    
...
if( $result ){
	while( $line = sql_fetch_array( $result )) {
		...
		$run = '../menu/tree_run.php?sys_pg=' . $sys_pg . '&sys_subtitS=' . $line['sys_subtit'] .'&open_mode=on&mid='.$mid. '&sys_jong=' . $sys_jong. '&num=' . $num.'&job_addr='.$job_addr;

...
<?php
	...
	if( isset($_POST['g_name']) ) $g_name = $_POST['g_name'];
	else $g_name ='';
	...
	if( $mode == 'insert_group1') {
		...
		$f_path  = $f_path1 . "/";
		$in_date = time();
		$file_ = preg_replace("/\s+/","", $_FILES["file"]["name"] );
		if( $_FILES["file"]["error"] > 0){
			...
			if( file_exists( $f_path . $file_)){
					echo $file_ . " I have the same file. ";
...
<!--
  ...
  function upload_func(no, i){
	...
	function update_func(no, i, g_code, g_name){
		...
		document.kapp_form.sel_g.value		= g_code+":"+g_name;
	...
	function img_ext(){
		...
		fm = ff1[cnt];
		ff = fm.split('.');
		document.kapp_form.jpg_name.value = ff[0];
		document.kapp_form.jpg_memo.value = ff[0];
	}
-->
</script>
...
<div id="mypaneltab" class="ddpaneltab" ><span style="background-color:;color:yellow;"><a href="#" style='height:25px;color:yellow;'>&nbsp; &#9776; My Image Insert &nbsp;▼ &nbsp;</a></span></div>

...
include_once('../tkher_start_necessary.php');
		...
		//echo "============ host:" . $host;
		/*
			소스 정 위치 : /t/manual/user_manual.php --> /t/manual.php : <--- 구버전위치
		*/
	...
	//125.184.157.158 //$urllinkcoin_my_ip

	//	if($ip != "125.184.157.158"){
	//	if($ip != $urllinkcoin_my_ip){
	//if($ip != "1.252.67.36"){
	if($ip != "125.184.157.214"){

		$agent  = getenv("HTTP_USER_AGENT");
		$Accept = getenv("HTTP_ACCEPT");
		$msg = $ip . "|" . $agent . "|" . $Accept;	//66.249.82.159|Mozilla/5.0 (Linux; Android 7.0; SM-G920L) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.98 Mobile Safari/537.36|text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8
		//89.191.156.102|Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36|
		$pattern = "/mobile/i";
		...
		$ret = sql_query("SELECT * from kapp_ip_info where ipno1 <= $ip_num and ipno2 >= $ip_num " );
		...
		//sql_query("INSERT into log_info SET name='guest', id='guest',ip='$ip', msg='$msg', type='$type' , country_cd='$ipcode', country_name='$ipname', uptime='$day', start_pg='manual' " );
	}
?>
<html>
<head>
...
img { width:100%; }
	...
	<script>
		function jumun()
		{
			var modeX = document.inform.mode.value='jumun_insert';
			var nameX = document.inform.nameX.value;
			var ethX = document.inform.ethX.value;
			var cntX = document.inform.cntX.value;

			if (document.inform.nameX.value == "") {
				alert("You can apply for donation after signing up.");// \n 로그인 후 기부신청이 가능합니다.
				document.inform.nameX.focus();
				return false;
			} else if(document.inform.ethX.value == "") {
				alert("Please enter your ETH Address.");	// \n  이더리움 지갑주소를 입력바랍니다.
				document.inform.ethX.focus();
				return false;
			} else if(document.inform.cntX.value < 1) {
				alert("You must enter 1ETH or more. ");	//\n  이더리움 1이상 입력해야 합니다.
				document.inform.cntX.focus();
				return false;
			} else {
				alert("OK mode=" + modeX);
				document.inform.action='manual.php';
...
<br><ul><li style='font-size:18px;'>Program-Make : 메인화면의 App Make에서 'Program-Make'를 클릭</li><!-- <a href="#top">Top</a> --></ul>
...
<img src='./pic_table_create.png' width='100%'><!-- pic_002.png -->
...
<img src='./db_set_screen.png' width='100%'><!-- pic_007.png -->
...
<img src='./pic_020.png' width='100%'><!-- table delete : pic_005.png-->
<ul style='color:white'><li> 설명 : 

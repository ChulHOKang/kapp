		...
		if( $filenm ) {
			...
			if ( ($result = sql_query( $SQL ) )==false )
			{
			  printf("Invalid query: Whole query: %s\n", $SQL);
				my_msg(" Update error occurred.");	// \\n Update 오류가 발생하였습니다. 
				exit();
			} else {
				my_msg(" The image file has been changed.");	// \\n 이미지화일를 변경 하였습니다. 
			}
		}
}
...
if($mode == "Update_func") {
	...
	if( $H_LEV < 8 ) {
		...
		if ( ($result = sql_query( $SQL ) )==false )
		{
		  printf("Invalid query: %s \n", $SQL);
			...
			my_msg(" Title, Message, display order changed.");	// \\n Title , Message, display 순서를 변경 하였습니다. 
		}
}
	...
	if( $H_LEV < 8 ) {
		...
		{
		  printf("Invalid query: %s \n", $SQL);
			my_msg("A delete error occurred.");	// \\n delete 오류가 발생하였습니다. 
			exit();
		} else {
			my_msg(" Deleted.");	// \\n 삭제 하였습니다. 
		}
	...
	if( $H_LEV < 8 ) {
		...
		$cd = 'main';		//$jpg[0];	
		$nm = 'main';		//$jpg[1];
//        $f_path = "../cratree/" . $H_ID . "/";	// --------------------------- tkher :보완필요.
        $f_path = KAPP_PATH_T_ . "/data/main_scroll_image/";	//---------------------------------- 보완 OK.
		$in_date = time();
		...
		if ( $_FILES["file"]["error"] > 0){ 
			echo "Return Code: " . $_FILES["file"]["error"] . "<br>"; 
				...
				{ 
						...
						{
							printf("Invalid query: %s \n", $SQL);
							my_msg("Registration error occurred.");	// \\n 등록 오류가 발생하였습니다. 
							//exit();
						} else {
							my_msg(" Registered.");	// \\n 등록 하였습니다. 
						}
...
<?php
	...
	if ( ($result = sql_query( $SQL ) )==false )
	{
	  ...
	  exit();
		...
		echo "	
			<tr>
				<td align='center'><img src='./data/main_scroll_image/".$row['jpg_file']."' width='300' height='200' title='$jpg_file'></td>
				<td align='left'>
		...
		echo "	&nbsp;
				...
				<td align='center'>
					...
					<input type=button value='Delete_Image' onclick=\"javascript:del_check('$no', '$i' )\" style='height:22px;background-color:red;color:white;border:1 solid black' title='Delete the image.'>
				</td>
			</tr>
			";

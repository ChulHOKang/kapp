...
function visit($skin_dir='basic')
{
    ...
    // $visit[4] = 전체
    // 숫자가 들어감
    preg_match("/오늘:(.*),어제:(.*),최대:(.*),전체:(.*)/", $config['kapp_visit'], $visit);
	...
	if( preg_match('#^theme/(.+)$#', $skin_dir, $match)) {
        if (KAPP_IS_MOBILE) {
            //$visit_skin_path = KAPP_THEME_MOBILE_PATH.'/'.KAPP_SKIN_DIR.'/visit/'.$match[1];
            $visit_skin_path = KAPP_PATH_T_ .'/visit/'.$match[1];
            if(!is_dir($visit_skin_path))
                $visit_skin_path = KAPP_PATH_T_ . '/visit/'.$match[1];
            $visit_skin_url = str_replace(KAPP_PATH_T_, KAPP_URL_T_, $visit_skin_path);
        } else {
            $visit_skin_path = KAPP_PATH_T_.'/visit/'.$match[1];
            $visit_skin_url = str_replace(KAPP_PATH_T_, KAPP_URL_T_, $visit_skin_path);
        }
        ...
        if(KAPP_IS_MOBILE) {
            $visit_skin_path = KAPP_PATH_T_ . '/visit/'.$skin_dir;
            //$visit_skin_url = KAPP_MOBILE_URL.'/'.KAPP_SKIN_DIR.'/visit/'.$skin_dir;
            $visit_skin_url = str_replace(KAPP_PATH_T_, KAPP_URL_T_, $visit_skin_path);
        } else {
            $visit_skin_path = KAPP_PATH_T_ . '/visit/'.$skin_dir;
            $visit_skin_url = KAPP_URL_T_ . '/visit/'.$skin_dir;
        }
    }
	ob_start();
    //include_once ( './visit.skin.php');//include_once ( $visit_skin_path.'/visit.skin.php');

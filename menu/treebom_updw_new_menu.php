<?php
	...
	*/
	...
	if ( $H_ID !== $mid ) {
		...
		//			$sys_link = "../t/menu/" . $sys_link; // memo에 메뉴 실행 url을 저장.
		//$rungo = "/cratree/" . $rs['sys_userid'] . "/". $sys_pg_root . "_runf.html";
		...
		//else $rungo = "/cratree/" . $rs['sys_userid'] . "/". $sys_pg_root . "_runf.html";

		echo "<script>window.open( '$rungo' , '_top', ''); </script>";
		exit;
	}

	$sys_subtit = "sys_subtit_0";
	$sys_subtit	= $_REQUEST[$sys_subtit];
    
	//m_("sys_subtit:$sys_subtit"); //sys_subtit:AI-반도체-NPU , sys_subtit:초전도체-LK99
...
<?php 
	for ( $intloop = 0; $sys_subtit; $intloop++ ) {
			...
			if ( $sys_subtit != "") { 
				...
				/* 2023-08-07 change 
				if( $typex == 'T' ) $type_nm='cratree';
				else if( $typex == 'B' ) $type_nm='BOOK';
				else if( $typex == 'M' ) $type_nm='popup';//popup menu
				else if( $typex == 'G' ) $type_nm='BBSTREE';
				else if( $typex == 'U' ) $type_nm='LINK'; 
				else  $type_nm='gita';*/
...
function sys_menu_bom_update_curl( $sys_pg ){
    ...
    //$url_ = 'https://ailinkapp.com/onlyshop/coupon/treebom_updw_curl_ailinkapp.php'; // 전송할 대상 URL
    $url_ = 'https://ailinkapp.com/kapp/_Curl/treebom_updw_curl_ailinkapp.php'; // 전송할 대상 URL

<?php
	...
	*/
	...
	if( $H_ID == '' ) {
		...
		echo "<meta http-equiv='refresh' content=0;url='tkher_program_data_list.php?pg_code=".$_POST['pg_code']."'>";exit;
	}
?>
<html>
<head>
...
<div>
	<P onclick="javascript:Kapp_ProgramJS.kapp_program_data_list(this.form);" class="HeadTitle02AX" title='pg:<?=$pg_code?>, tab:<?=$tab_enm?> '><?=$pg_name?></P>
</div>
...
<?php
		...
		if ( $resultPG == false ) { m_(" kapp_program_data_update pg_name:$pg_name select ERROR "); exit; }
	...
	for( $i=0,$j=1; isset($list[$i]) && $list[$i] != ""; $i++, $j++ ){
		...
		if( $fld[3] == "TEXT" ) {
			...
			echo " <div class='blankA'> </div> ";
				...
				echo " <div class='menu1T' align=center><span style='width:$Xwidth;height:$Xheight;'>$fld[2]</span></div> ";
				echo " <div class='menu1A'><input type='text' name='$fldenm' value='$row[$fldenm]' onclick=\"javascript:Kapp_ProgramJS.popup_call(this.form, $i)\" style='width:$Xwidth;height:$Xheight;' placeholder='PopUp Window. Please enter a $fld[2].'></div> ";
				echo " <div class='blankA'> </div> ";
			...
			if( $row[$fldenm] != '' ) {
					...
					if( strtolower($ifile[1]) == 'jpg' || strtolower($ifile[1]) == 'png' || strtolower($ifile[1]) == 'gif' ) {
						echo"<p>$fldhnm</p>";
						echo"<div class='viewWriteBox' ><a href='#' onClick=\"Kapp_ProgramJS.popimageV('$im',$image_size[0],$image_size[1]);return false\" onfocus='this.blur()'><img src='$im'  width='400' height='300' border=0></a> </div>";
...
?>
				...
				<div class="viewHeader">
					<a href="javascript:Kapp_ProgramJS.kapp_program_data_modify(this.form, <?=$seqno?>);" class="btn_bo02"> Submit </a>
					<a href="javascript:Kapp_ProgramJS.kapp_program_data_list(this.form);" class="btn_bo02">List</a>
				</div>
			</form>
			</div>
<?php
...
<?php
	...
	if( $mode == 'CHG_MODE' ){
			...
			IF( $i==($item_cnt-1) ) { // 마지막 컬럼 체크 "," 처리를 위해...sql a=1, b=2 
				if( $typeX=='3' ) {
					...
					if( isset($upfile_name) && $upfile_name !='' ) {
						...
						if( $_FILES["$nm"]["error"] > 0){ // error check
							echo "kapp_program_data_update nm:$nm, Return Code: " . $_FILES["$nm"]["error"] . "<br>"; // fld_3
						} else { // none error
				...
				if( $typeX=='3' ) {				// 3: checkbox
					...
					if( isset($upfile_name) && $upfile_name !='' ) {
						...
						$SQL = $SQL . $fld[1] ."= '" .$upfile_name. "', ";
						if( $_FILES["$nm"]["error"] > 0){ // error check
							echo "kapp_program_data_update nm:$nm, Return Code: " . $_FILES["$nm"]["error"] . "<br>";
						} else { // none error
							move_uploaded_file($_FILES["$nm"]["tmp_name"], $f_path.$upfile_name );
						}
					}
		...
		$ret = sql_query( $SQL ) or die ("kapp_program_data_update.php Error sql:" . $SQL);
		if( $ret ) {
